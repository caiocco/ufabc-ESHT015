...
%
    ...
    Dados de 2016 do \glsdesc{snis} indicam que apenas 40\% da população urbana é atendida por esgotamento sanitário. Ainda, não aponta destinação para o esgotamento.

    De acordo com o relatório do Plano de Bacia Hidrográfica do Alto Tietê, elaborado pela \gls{fabhat}, o esgoto é integralmente lançado \textit{in natura} em corpos d'água.
    
    \subsection{Restrições à ocupação}
    
    Um dos grandes fatores para a manutenção do ciclo de pobreza no município é a grande limitação geológica que o caracteriza. Devido ao relevo formado predominantemente por escarpas, serras e morrotes, os habitantes enfrentam grandes desafios em construir suas casas. 

    De acordo com a Cartas de Suscetibilidade a Movimentos de Massa elaborada pelo \gls{ipt}, cerca de 32\% da área urbanizada do município se enquadra nas categorias de alta e média suscetibilidade. 
    
    \begin{figure}[H]
		\centering
		\caption{Suscetibilidade a movimento de massa e áreas ocupadas}
		\includegraphics[width=\linewidth]{img/DESLIZAMENTOF5.png}
		\label{fig:movimento_massa}
		\legend{Elaboração própria. Fonte: Limites municipais (CEM, 2015) e Carta de Suscetibilidade de Franscisco Morato (IPT, 2014)}
	\end{figure}
	
	\begin{figure}[!ht]
		\centering
		\caption{Suscetibilidade a inundação e proposta de área de preservação permanente}
		\includegraphics[width=\linewidth]{img/APPeInundacao2.png}
		\label{fig:inundacao}
		\legend{Elaboração própria. Fonte: Hidrografia (CEM, 2015) e Carta de Suscetibilidade de Franscisco Morato (IPT, 2014)}
	\end{figure}
	
	% TODO
	% Jade, coloque a URL dos arquivos usados pra montar as figuras, pois quando eu chegar na UFABC, coloco nas referências
    % mando no chat 
